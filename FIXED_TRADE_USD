...
def process_symbol(exchange, symbol, state):
    ...
    try:
        ...
        # Order failed (e.g. insufficient balance) - still notify via Telegram
        # instead of only leaving it in the GitHub Actions log. Position is
        # NOT marked open, and last_candle is intentionally not updated so
        # the same candle can be retried if the failure was transient.
        fail_msg = (
            f"⚠️ سیگنال {signal} روی {symbol} (M5) صادر شد ولی سفارش ثبت نشد\n"
            f"{base_info}\n"
            f"دلیل خطا: {e}"
        )
        try:
            ...
            print(f"[{symbol}] Also failed to send Telegram failure notice: {tg_err}")
        print(f"[{symbol}] Order failed: {e}", file=sys.stderr)
        return
